...
%% v=3
v=3
t_1  = V3.data(1,666:948)'-V3.data(1,666); 
t_2 = V3.data(1, 666:948)'-V3.data(1,666);
...
gamma_1 = beta/ alpha;
gamma_2 = 1 - gamma_1;%% State matrix representation
v_pump  = 3;
gain_pump = 12.877;
gamma_1 = 0.3603;
gamma_2 = 1 - gamma_1;
...
g = 981;            % cm/s^2
q_total = gain_pump * v_pump;   % total pump flow, cm^3/s

% --- Correct equilibrium points ---
q1    = (1 - gamma_1) * q_total;
h1_eq = (q1 / (A_D1 * sqrt(2*g)))^2;

q2    = gamma_2 * q_total + A_D1 * sqrt(2*g*h1_eq);
h2_eq = (q2 / (A_D2 * sqrt(2*g)))^2;

fprintf('h1_eq = %.3f cm\n', h1_eq);
fprintf('h2_eq = %.3f cm\n', h2_eq);

% --- Correct Jacobian coefficients ---
k1 = (A_D1 / S) * sqrt(g / (2*h1_eq));
k2 = (A_D2 / S) * sqrt(g / (2*h2_eq));

% --- State space matrices ---
A_sys = [-k1,  0;
          k1, -k2];

B_sys = gain_pump * [(1 - gamma_1) / S;
                           gamma_2  / S];

C_sys = [0, 1];     % output is L2 only
D_sys = 0;

% --- Sanity checks ---
sys = ss(A_sys, B_sys, C_sys, D_sys);
disp('Open-loop poles:'); disp(eig(A_sys));
disp('System is controllable:'); disp(rank(ctrb(A_sys, B_sys)) == 2);